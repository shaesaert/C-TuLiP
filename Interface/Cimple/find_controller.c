//
// Created by be107admin on 7/29/17.
//
...
void gsl_matrix_from_array(gsl_matrix *matrix, size_t n, size_t m, double array[]){
    for(size_t i = 1; i < n+1; i++){
        for(size_t j = 1; j<m+1; j++){
            gsl_matrix_set(matrix, i, j, array[j+m*(i-1)]);
        }
    }
};
...
polytope solve_feasible_closed_loop(polytope *P1, polytope *P2, system_dynamics *s_dyn){
    ...
    gsl_vector_view G_P1 = gsl_vector_subvector(precedent_polytope.G, 1, P1->G->size);
    gsl_vector_memcpy(&G_P1.vector, P1->G);
    gsl_vector_view G_P2 = gsl_vector_subvector(precedent_polytope.G, P1->G->size+1,P2->G->size);
     ...
     *         |  0   |
     */
    gsl_matrix_view Dist_P2 = gsl_matrix_submatrix(Dist, P1->H->size1+1, 1, P2->H->size1, p);
    ...
    gsl_matrix_view H_P1 = gsl_matrix_submatrix(precedent_polytope.H, 1, 1, P1->H->size1, n);
    gsl_matrix_memcpy(&H_P1.matrix, P1->H);
    gsl_matrix_view H_P2_1 = gsl_matrix_submatrix(precedent_polytope.H, P1->H->size1+1, 1, P2->H->size1, n);
    gsl_blas_dgemm(CblasNoTrans,CblasNoTrans, 1.0, P2->H, s_dyn->A, 0.0, &H_P2_1.matrix);
    gsl_matrix_view H_P2_2 = gsl_matrix_submatrix(precedent_polytope.H, P1->H->size1+1, n+1, P2->H->size1, m);
    gsl_blas_dgemm(CblasNoTrans,CblasNoTrans, 1.0, P2->H, s_dyn->B, 0.0, &H_P2_2.matrix);

    if (s_dyn->U_set.H->size2 == m){
        gsl_matrix_view HU = gsl_matrix_submatrix(precedent_polytope.H,sum_dim+1, n+1, s_dyn->U_set.H->size1,m);
        ...
        gsl_matrix_view eye_m = gsl_matrix_submatrix(exchange_matrix, 1, n+1, m, m);
        gsl_matrix_set_identity(&eye_m.matrix);
        gsl_matrix_view eye_n = gsl_matrix_submatrix(exchange_matrix, m+1, 1, n, n);
    ...
    if (!(gsl_matrix_isnull(Dist))){
        gsl_matrix * maxima = gsl_matrix_alloc(Dist->size1, s_dyn->L_M_helper.D_extremes->size2);
        //Calculate Dist.Dextremes (extremum of each dimension of each polytope)
        gsl_blas_dgemm(CblasNoTrans, CblasNoTrans, 1.0, Dist, s_dyn->L_M_helper.D_extremes,0.0, maxima);
...
void set_path_constraints(system_dynamics * s_dyn, polytope list_polytopes[], size_t N){
////////////////////////////////////////////////////////////////////////////////
...
/* INITIALIZE MATRICES: Lk, Mk, Gk, H_diag*/
    gsl_matrix_view  Lk = gsl_matrix_submatrix(s_dyn->L_M_helper.L, 1, 1,sum_polytope_dim,n+N*m);
    /*
     *                          |H_0        0          0          0          0  |
     *                          |H_1.A    H_1.B        0          0          0  |
     *Lk =  H_diag.L_default  = |H_2.A^2  H_2.A.B    H_2.B        0          0  | dim[sum_polytope_dim x n*N]
     *                          |H_3.A^3  H_3.A^2.B  H_3.A.B    H_3.B        0  |
     *                          |H_4.A^4  H_4.A^3.B  H_4.A^2.B  H_4.A.B    H_4.B|
     *
     *   with L = |Lk|
     *            |Lu|
     * */
    //Reset Lk
    gsl_matrix_set_zero(&Lk.matrix);

    gsl_vector_view Mk = gsl_vector_subvector(s_dyn->L_M_helper.M, 1, sum_polytope_dim);
     ...
     *                          |0            0          0        0      0|
     *                          |H_1.E        0          0        0      0|
     *Gk =  H_diag.L_default  = |H_2.A.E    H_2.E        0        0      0| dim[sum_polytope_dim x p*N]
...
/*Update L and M*/
    // Lk = H_diag.L_default
    gsl_blas_dgemm(CblasNoTrans,CblasNoTrans,1.0, H_diag, s_dyn->L_M_helper.L_default,0.0,&Lk.matrix);

    // Gk = H_diag.E_default
    gsl_blas_dgemm(CblasNoTrans,CblasNoTrans,1.0, H_diag, s_dyn->L_M_helper.E_default,0.0,Gk);

    /*Find maxima of Gk.Dextremes*/
    //TODO: if Gk non zero else...
    gsl_matrix * maxima = gsl_matrix_alloc(Gk->size1, s_dyn->L_M_helper.D_extremes->size2);
    //Calculate Gk.Dextremes (extremum of each dimension of each polytope)
    gsl_blas_dgemm(CblasNoTrans, CblasNoTrans, 1.0, Gk, s_dyn->L_M_helper.D_extremes,0.0, maxima);
...
////////////////////////////////////////////////////////////////////////////////
    ...
    if (closed_loop == 1) {
        ...
        set_path_constraints(s_dyn, polytope_list, N);
    }else {
        for (int i = 1; i < N; i++) {
            polytope_list[i] = P1;
        }
    ...
    gsl_matrix_view L_x = gsl_matrix_submatrix(s_dyn->L_M_helper.L, P1.H->size1+1, 1, (s_dyn->L_M_helper.L->size1)-(P1.H->size1), n);
    // L_u = L[{(polytope[0]+1),(dim_n(L))},{(n+1),(dim_m(L))}]
    gsl_matrix_view L_u = gsl_matrix_submatrix(s_dyn->L_M_helper.L, P1.H->size1+1, n+1, (s_dyn->L_M_helper.L->size1)-(P1.H->size1), s_dyn->L_M_helper.L->size2 - n);
    // M_view = M[{(polytope[0]+1),(dim_n(M))}]
    gsl_vector_view M_view = gsl_vector_subvector(s_dyn->L_M_helper.M, P1.H->size1+1, s_dyn->L_M_helper.M->size-P1.H->size1);
