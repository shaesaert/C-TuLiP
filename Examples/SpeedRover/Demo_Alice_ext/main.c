      ...
      else
      {
        ...
        do
        {
           ...
           if (word)
           {
             ...
             // Just want to pass through numbers not hex here...
             //event->data[i] = strtoul(word, NULL, 16);
             event->data[i] = strtoul(word, NULL, 10);
           }
           // printf("word[%d] = %s\n", i, word);
           i = i + 1;
        } while (word);
...
void receiveCmd(char *cmdBuf)
{
  // The scanf ends at a space so cannot work with extra data args
  //scanf("%s", cmdBuf);
  fgets(cmdBuf, SOCK_BUFF_SIZE, stdin);
}
